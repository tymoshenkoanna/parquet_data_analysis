...
# define paths
file_pattern = '/Users/annatymoshenko/Desktop/Docs/Data Analysis/Backlink_check/input_parquet/*.parquet'
excel_path = '/Users/annatymoshenko/Desktop/Docs/Data Analysis/Backlink_check/input_fol_links_sistrix.xlsx' 
#output_path = '/Users/annatymoshenko/Desktop/Docs/Data Analysis/Backlink_check/output_fol_sistrix_cleaned.xlsx' 
output_path_csv='/Users/annatymoshenko/Desktop/Docs/Data Analysis/Backlink_check/output_fol_parquet_sistrix_merged.csv' 
...
query = (
    ...
    # 3. emove any duplicates created by the join, if there is a raw that has same values from_url and in "relevant_follow_links" it will be deleted
    .unique(subset=["from_url", "relevant_follow_links"], keep="first")
    # 4. get the document ID (number) from the URL
    ...
    # 5. keep only rows where an ID was successfully found, everything else doesn't interest me now
    .filter(pl.col("extracted_id").is_not_null())
    # 6. final column selection and order
    .select([
        "from_url_id", "source_pair_id", "from_url", "from_domain",
        "status_code", "robots_content", "language", "relevant_follow_links",
...
# ensure the folder for the output file is there
...
# save all the data in the final file
